 ...
 *
 * @attention Must be called after invoking IMP_FrameSource_EnableChn.
 */
...
typedef struct {
	uint8_t position[12][12];	//Concatenate the checkerboard pattern, before configuration, please memset it to 0xff first. Then arrange it compactly from the top left corner. Do not allow crossing grids or rows. The 0th position is the output channel for the concatenation.
	int16_t width;				//Get the output parameter of the width after joint.
	int16_t height;				//Get the output parameter of the height after joint.
	int32_t handler;			//Get the output parameter of the handler after joint.
}IMPFSJointAttr;

/**
 * @brief int IMP_FrameSource_CreateJoint(IMPFSJointAttr *joint_attr)
 *
 * Create FrameSource joint.
 *
 * @param[in] position[12][12] joint checkerboard.
 * @param[out] handler joint handler.
 * @param[out] width   joint width.
 * @param[out] height  joint height.
 *
 * @retval 0 success
 * @retval non-0 failure，its value is an error code
 *
 ...
 * }
 *
 * //Incorrect usage(block spacer), Channel 4 is lost and will not be joint.
 * position[12][12] = {
 * {0,    3,    0xff, 0xff},
 * {1,    0xff, 4,    0xff},
 * {0xff, 0xff, 0xff, 0xff},
 ...
 * }
 *
 * //Incorrect usage(line spacer),Channel 4 is lost and will not be joint.
 * position[12][12] = {
 * {0,    3,    0xff, 0xff},
 * {1,    0xff, 0xff, 0xff},
 * {0xff, 0xff, 0xff, 0xff},
 ...
 * @endcode
 *
 * @attention After calling IMP_FrameSource_CreateChn, and before calling IMP_FrameSource_EnableChn.
 */
 ...
 *
 * Obtain the concatenation information of FrameSource through the handle.
 *
 * @param[out] position[12][12] joint checkerboard.
 * @param[in] handler joint handler.
 *
 * @retval 0 success
 * @retval non-0 failure，its value is an error code
 *
 * @attention After IMP_FrameSource_CreateJoint
 */
