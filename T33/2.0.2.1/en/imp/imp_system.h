 ...
 *
 * @param[in] poolId:  Release memory from the mempool rmem.
 *
 * @retval:  0, success
 * @retval:  < 0, failure
 *
 * @attention: Memory pool release is prohibited while the stream is running.
 * It can only be performed after all resources have been released.
 * Also note that in the SDK, some memory allocations do not use the memory pool.
 * Frequent allocation and release may cause allocation failures due to memory fragmentation.
 * In general, it is recommended to release the memory pool only when the rmem
 * memory is needed for other purposes.
 */
