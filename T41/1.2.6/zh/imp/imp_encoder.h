...
typedef enum {
	IMP_ROI_QPMODE_DELTA         = 0,  /**< ROI相对QP模式 */
	IMP_ROI_QPMODE_FIXED_QP      = 1,  /**< ROI绝对QP模式，暂不支持 */
...
typedef struct {
	...
	int8_t                       qp;   /**< mode=IMP_ROI_QPMODE_DELTA,相对qp [-26~25]; 暂不支持绝对QP */
} IMPEncoderRoiWin;
...
typedef struct {
	...
	int                          mapSize;
	IMPEncoderQPMode             mode;
	int8_t                       Quality;    /**< mode = IMP_ROI_QPMODE_DELTA,相对qp [-26~25]; 暂不支持绝对QP */
	int32_t                      reserved;
 ...
 * @retval 非0 失败
 *
 * @remarks 此API只适用于宽32对齐,高16对齐的NV12输入编码JPEG
 *
 * @attention 无
 */
 ...
 *
 * @attention:
 * pstMapRoi->map      map数组首地址.
 * pstMapRoi->mapSize  map数组的长度.
 * pstMapRoi->mode     ROI_QP模式.
 * pstMapRoi->Quality  Qp值.若mode=IMP_ROI_QPMODE_DELTA，ROI_QP为相对qp[-26~25].暂不支持绝对QP.
 *
 * mapSize与H264/h265以及编码分辨率有关:
 * ROI模式下:H265一个宏块为32*32，H264一个宏块为16*16.以h265/1920*1080为例，mapSize = 1920/32 * 1080/32 = 60*34 = 2040.
 * map为数组首地址，是将一帧图片分成mapSize长度的数组.详细说明如下。
 *
 * map[i]其中高6bit代表QP值，低2bit表示模式.
 * 低2bit:
 * 00:不使用Map_Roi模式，高6bit无效.
 * 01:使用Map_Roi模式，所有宏块QP值是入参MapRoi->Quality，高6bit无效.
 * 10:使用Map_Roi相对模式，当前宏块QP由map的高6bit决定，使用补码表示法.
 *
 * 高6bit:
 * 1、使用Map_Roi相对模式时取值范围[-32,31]，使用补码表示法来表示正负数:111111=-1;111110=-2;111101=-3;...;100000=-32;
 * 取值为32,33,34,...,63;对应相对QP:-32,-31,...,-1;
 * 取值为0,1,2,...,31;对应相对QP:0,1,2,...,31;
 *
 * 例如:
 * 三个宏块使用Map_Roi相对模式，相对Qp值为-1，-2，15.
 * map[i] = (63 << 2) | 0x2; map[i+1] = (62 << 2) | 0x2; map[i+2] = (15 << 2) | 0x2;
 *
 * 参考sample-Encoder-video-Roi.c
 */
