 ...
 * int ret = 0;
 * IMPISPAeAtList attr;
 * IMP_ISP_Tuning_GetAeAtList(IMPVI_MAIN,  &attr);
 * if(ret){
 ...
 * @code
 * int ret = 0;
 * IMPISPAeEvList attr1;
 * IMP_ISP_Tuning_GetAeEvList(IMPVI_MAIN,  &attr1);
 * if(ret){
 *		IMP_LOG_ERR(TAG, "IMP_ISP_Tuning_GetAeAtList error !\n");
 *		return -1;
 * }
 ...
 */
typedef struct {
	uint16_t gpio_num[10];		/**< gpio端口,需要申请或释放的GPIO端口，以0xFF结束 */
	uint16_t gpio_sta[10];		/**< gpio状态,申请GPIO的初始化状态，0：低 1：高 */
    uint16_t free;				/**<  0：申请 1：释放 */
} IMPISPGPIO;
 ...
 *
 *	attr.ae_step[0] = 50;//[0][1]白天起始收敛速度
 *	attr.ae_step[1] = 50;
 *	attr.ae_step[2] = 50;//白天到黑夜
 *	attr.ae_step[3] = 50;//黑夜到白天
 *
 ...
 *
 *	attr.awb_step[0] = 10;//收敛平滑帧数
 *	attr.awb_step[1] = 10;//初始rg
 *	attr.awb_step[2] = 10;//初始bg
 *	attr.awb_step[3] = 10;//初始rg
 *	attr.awb_step[4] = 10;//初始bg
 *
