...
/*
 * Set the GPIO status in the VIC interrupt.
 */
typedef struct {
	uint16_t gpio_num[10];		/** gpio port. The GPIO port to be applied for or released ends in 0xFF. */
	uint16_t gpio_sta[10];		/** gpio state. Indicates the initialization status of GPIO. 0: low 1: high. */
    uint16_t free;
} IMPISPGPIO;

/**
 * @fn int32_t int32_t IMP_ISP_SET_GPIO_INIT_OR_FREE(IMPVI_NUM num, IMPISPGPIO *attr)
 *
 * Apply for or release GPIO resources.
 *
 * @param[in] num Indicates the label of the sensor
 * @param[out] attr attribute
 *
 ...
 * @retval non-0 fails, returns an error code
 *
 * @remark gpio_num[10]={20,21,0xff},gpio_sta[10]= {1,0} Example Initialize the PA20 output to high PA21 output to low
 *
 * @attentionBefore this function is used, IMP_ISP_EnableTuning is called.
*/
int32_t IMP_ISP_SET_GPIO_INIT_OR_FREE(IMPVI_NUM num, IMPISPGPIO *attr);

/**
 * @fn IMP_ISP_SET_GPIO_STA(IMPVI_NUM num, IMPISPGPIO *attr)
 *
 * Next VIC DONE set the GPIO status.
 *
 * @param[in] num Indicates the label of the sensor
 * @param[out] attr attribute
 *
 ...
 * @attentionBefore this function is used, IMP_ISP_EnableTuning is called.
 */
int32_t IMP_ISP_SET_GPIO_STA(IMPVI_NUM num, IMPISPGPIO *attr);
...
typedef struct {
	uint16_t ae_step[4];
} IMPISPAeConvergeStep;

/**
 * @fn int32_t IMP_ISP_Tuning_SetAeConvergeStep(IMPVI_NUM num, IMPISPAeConvergeStep *converge_step)
 *
 * Set the convergence step of AE
 *
 * @retval 0 Success
 * @retval non-0 fails, returns an error code
 *
 ...
 * @attentionBefore this function is used, IMP_ISP_EnableTuning is called.
 */
int32_t IMP_ISP_Tuning_SetAeConvergeStep(IMPVI_NUM num, IMPISPAeConvergeStep *converge_step);

/**
 * @fn int32_t IMP_ISP_Tuning_GetAeConvergeStep(IMPVI_NUM num, IMPISPAeConvergeStep *converge_step);
 *
 * Get the convergence step of AE
 *
 * @retval 0 Success
 * @retval non-0 fails, returns an error code
 *
 * @code
 *	IMPISPAeConvergeStep attr;
 *	memset(&attr, 0, sizeof(IMPISPAeConvergeStep));
 *	IMP_ISP_Tuning_GetAeConvergeStep(IMPVI_MAIN,&attr);
 *	printf("[%s %d] [ae %d %d %d %d]\n", __func__, __LINE__,
 *				attr.ae_step[0], attr.ae_step[1], attr.ae_step[2], attr.ae_step[3]);
 *
 *	attr.ae_step[0] = 50;//[0][1]Convergence rate at the beginning of the day
 *	attr.ae_step[1] = 50;
 *	attr.ae_step[2] = 50;//Day to night
 *	attr.ae_step[3] = 50;//Night to day
 *
 *	IMP_ISP_Tuning_SetAeConvergeStep(IMPVI_MAIN,&attr);
 *	IMP_ISP_Tuning_GetAeConvergeStep(IMPVI_MAIN,&attr);
 *	printf("[%s %d] [ae %d %d %d %d]\n", __func__, __LINE__,
 *				attr.ae_step[0], attr.ae_step[1], attr.ae_step[2], attr.ae_step[3]);
 * @endcode
 ...
 * @attentionBefore this function is used, IMP_ISP_EnableTuning is called.
 */
int32_t IMP_ISP_Tuning_GetAeConvergeStep(IMPVI_NUM num, IMPISPAeConvergeStep *converge_step);

typedef struct {
	uint32_t awb_step[5];
} IMPISPAwbConvergeStep;

/**
 * @fn int32_t IMP_ISP_Tuning_SetAwbConvergeStep(IMPVI_NUM num, IMPISPAwbConvergeStep *converge_step)
 *
 * Set the convergence step of AWB
 *
 * @retval 0 Success
 * @retval non-0 fails, returns an error code
 *
 ...
 * @attentionBefore this function is used, IMP_ISP_EnableTuning is called.
 */
int32_t IMP_ISP_Tuning_SetAwbConvergeStep(IMPVI_NUM num, IMPISPAwbConvergeStep *converge_step);

/**
 * @fn int32_t IMP_ISP_Tuning_GetAwbConvergeStep(IMPVI_NUM num, IMPISPAwbConvergeStep *converge_step);
 *
 * Get the convergence step of AWB
 *
 * @retval 0 Success
 * @retval non-0 fails, returns an error code
 *
 * @code
 *	IMPISPAwbConvergeStep attr;
 *	memset(&attr, 0, sizeof(IMPISPAwbConvergeStep));
 *	IMP_ISP_Tuning_GetAwbConvergeStep(IMPVI_MAIN,&attr);
 *	printf("[%s %d][awb [%d %d %d %d %d]\n", __func__, __LINE__,
 *				attr.awb_step[0], attr.awb_step[1], attr.awb_step[2], attr.awb_step[3], attr.awb_step[4]);
 *
 *	attr.awb_step[0] = 10;//Convergence smoothing frames
 *	attr.awb_step[1] = 10;//Initial RG
 *	attr.awb_step[2] = 10;//Initial BG
 *	attr.awb_step[3] = 10;//Initial RG
 *	attr.awb_step[4] = 10;//Initial BG
 *
 *	IMP_ISP_Tuning_SetAwbConvergeStep(IMPVI_MAIN,&attr);
 *	IMP_ISP_Tuning_GetAwbConvergeStep(IMPVI_MAIN,&attr);
 *	printf("[%s %d][awb [%d %d %d %d %d]\n", __func__, __LINE__,
 *				attr.awb_step[0], attr.awb_step[1], attr.awb_step[2], attr.awb_step[3], attr.awb_step[4]);
 * @endcode
 ...
 * @attentionBefore this function is used, IMP_ISP_EnableTuning is called.
 */
int32_t IMP_ISP_Tuning_GetAwbConvergeStep(IMPVI_NUM num, IMPISPAwbConvergeStep *converge_step);
